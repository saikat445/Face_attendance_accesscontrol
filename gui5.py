    ...
    def append_objs_to_img(self,cv2_im2,frame):
        ...
        for encodeFace,faceLoc in zip(encodesCurFrame,facesCurFrame):
            ...
            if matches[matchIndex]:
                ...
                print( "attendance successfull" + name)
                #attendance.sort_values("timestamp", inplace=True)
                #attendance = attendance.drop_duplicates(subset="timestamp",keep = False, inplace = True)
                #print(("later-")+ attendance2)
                #attendance = attendance.drop_duplicates()

                attendance = attendance.drop_duplicates(subset=['name'], keep='first')
                fileName = "Attendance\Attendance.csv"
                attendance.to_csv(fileName, mode='a', header=None)
